...
int execRequest(int indexOfUser, int clientSockfd, UserID *listOfUsers, char *dirPath, int numberOfUsers, int *authSockets);
int authenticate(int clientSockfd, int numberOfUsers, UserID *listOfUsers, char *dirPath);
int sendWelcomeMsg(int clientSockfd);
int usersOnLine(int clientSockfd, UserID *listOfUsers, int numberOfUsers, int *authSockets);
...
int countFilesInDir(char* dirPath){
	...
	while ((entry = readdir(dirp)) != NULL) {
		if (entry->d_name[0] == '.' || !strcmp(entry->d_name, "Messages_received_offline.txt") || !strcmp(entry->d_name, "Messages_received_offline.txt~"))
			continue;
		file_count++;
	}
	closedir(dirp);
...
int activateServer(char *dirPath, int numberOfUsers, int port, UserID *listOfUsers) {
	...
	int sockfd, newsockfd, status, enable=1, i;
	struct sockaddr_in serverAddr, clientAddr;
	socklen_t addr_size = sizeof(clientAddr);
	int authSockets[numberOfUsers];
	int unAuthSockets[MAX_UNAUTHENTICATED_SOCKETS];
	fd_set readfds;
	int nfds = 0;
	int userIdx;
	...
	for(i=0; i<MAX_UNAUTHENTICATED_SOCKETS; ++i){
		unAuthSockets[i]=-1;
	}
	...
	while(1){
		...
		for(i=0; i<numberOfUsers; ++i){
			if(authSockets[i]!=-1){
				FD_SET(authSockets[i], &readfds);
				nfds = authSockets[i] > nfds ? authSockets[i] : nfds;
			}
		}
		for(i=0; i<MAX_UNAUTHENTICATED_SOCKETS; ++i){
			if(unAuthSockets[i]!=-1){
				FD_SET(unAuthSockets[i], &readfds);
				nfds = unAuthSockets[i] > nfds ? unAuthSockets[i] : nfds;
			}
		}
		nfds++;
		...
		if(FD_ISSET(sockfd, &readfds)){
			...
			if (newsockfd < 0 || sendWelcomeMsg(newsockfd)) {
				printf("accept() failed: %s\n", strerror(errno));
			}
			else{
				for(i=0; i<MAX_UNAUTHENTICATED_SOCKETS; ++i){
					if(unAuthSockets[i]==-1){
						unAuthSockets[i]=newsockfd;
						break;
					}
				}
			}
		...
		for(i=0; i<numberOfUsers; ++i){
			if(authSockets[i] != -1 && FD_ISSET(authSockets[i], &readfds)){
				if(execRequest(i, authSockets[i], listOfUsers, dirPath, numberOfUsers, authSockets)!=0){
					close(authSockets[i]);
					authSockets[i] = -1;
				}
			}
		...
		for(i=0; i<MAX_UNAUTHENTICATED_SOCKETS; ++i){
			if(unAuthSockets[i] != -1 && FD_ISSET(unAuthSockets[i], &readfds)){
				userIdx = authenticate(unAuthSockets[i], numberOfUsers, listOfUsers, dirPath);
				if(userIdx >= 0){
					authSockets[userIdx] = unAuthSockets[i];
				}
				else{
					close(unAuthSockets[i]);
				}
				unAuthSockets[i] = -1;
			}
		}
...
int createSubDirectories(char *dirPath, int numberOfUsers, UserID *listOfUsers) {
	...
	for(i = 0; i < numberOfUsers; ++i) {
		...
		strcat(userDirPath, "/Messages_received_offline.txt");	
		fp = fopen(userDirPath, "a");
		if(fp){
			fclose(fp);
		}
		else{
...
int authorizeUser(int clientSockfd, int numberOfUsers, UserID *listOfUsers, message *m, message *s, int *indexOfUser, char* dirpath){
	int i, userNameStartIndex=0, passwordStartIndex=0;
	message t;
	...
	for(i = strlen("User:"); i < strlen(m->data); ++i){
		if(!isspace(m->data[i])){
			userNameStartIndex = i;
			break;
		}
	}
	
	sprintf(buff, "%s", s->data);
	buff[strlen("Password:")] = '\0';
	if (strcmp(buff, "Password:") != 0){
		printf("ERROR : the message does not match the protocol\n");
		return ERROR_STATUS ;
	}
	
	for(i = strlen("Password:"); i < strlen(s->data); ++i){
		if(!isspace(s->data[i])){
	...
	for(i = 0; i < numberOfUsers; ++i) {
		if ( (strcmp((m->data)+userNameStartIndex, listOfUsers[i]->userName) == 0) && (strcmp((s->data)+passwordStartIndex, listOfUsers[i]->password)) == 0 ) {
			*indexOfUser = i;
...
int listOfFiles(int clientSockfd, char *dirPath, char *userName) {
	...
	if (d != NULL) {
		while ((dir = readdir(d)) != NULL) {
			// skip
			if (dir->d_name[0] == '.' || !strcmp(dir->d_name, "Messages_received_offline.txt") || !strcmp(dir->d_name, "Messages_received_offline.txt~"))
				continue;
...
int getMsgParams(char *msgBuf, int *idxRcvUsr, message m, UserID *listOfUsers, int numberOfUsers){
	
	char rcvUsrName[MAX_ID_CHAR];
	int i;
	
	if(sscanf(m.data, "%*s %s %[^\t\n]", rcvUsrName, msgBuf) < 2){
		printf("ERROR : sscanf encountered an error when parsing message to be sent\n");
		return ERROR_STATUS;
	}
	if(rcvUsrName[strlen(rcvUsrName)-1]!=':'){
		printf("ERROR : sscanf encountered an error when parsing message to be sent\n");
		return ERROR_STATUS;
	}
	rcvUsrName[strlen(rcvUsrName)-1]='\0';
	
	for(i=0; i<numberOfUsers; i++){
		if(strcmp(rcvUsrName, listOfUsers[i]->userName)==0){
			*idxRcvUsr = i;
			break;
		}
		if(i==numberOfUsers-1){
			printf("ERROR : didn't find the user to send message to\n");
			return ERROR_STATUS;
		}
	}
...
int transmitMsg(int rcvSkt, int indexOfUser, char *msgBuf, UserID *listOfUsers){
	
	message m;
	m.header.opcode = SEND_MESSAGE;
	m.data[0]=0;
	strcat(m.data, "New message from ");
	strcat(m.data, listOfUsers[indexOfUser]->userName);
	strcat(m.data, ": ");
	strcat(m.data, msgBuf);
	m.header.length = strnlen(m.data, MAX_DATA_SIZE)+1;
	
	if(sendMessage(rcvSkt, &m)) {
		return ERROR_STATUS;
	}
...
int writeMsgToFile(char *msgBuf, int idxRcvUsr, int idxSndUser, UserID *listOfUsers, char *dirPath){

	char msgsFilePath[MAX_DATA_SIZE];
	char m[MAX_DATA_SIZE];
	FILE *fp;
	
	strcpy(msgsFilePath, dirPath);
	strcat(msgsFilePath, "/");
	strcat(msgsFilePath, listOfUsers[idxRcvUsr]->userName);
	strcat(msgsFilePath, "/Messages_received_offline.txt");
	fp = fopen(msgsFilePath, "a");
	if(!fp){
		return ERROR_STATUS;
	}
	
	strcpy(m, "Message received from ");
	strcat(m, listOfUsers[idxSndUser]->userName);
	strcat(m, ": ");
	strcat(m, msgBuf);
	strcat(m, "\n");
	
	if(fwrite(m, sizeof(char), strlen(m), fp) != strlen(m)){
		printf("ERROR: writing offline message to file\n");
		fclose(fp);
		return ERROR_STATUS;
	}
	fclose(fp);
...
int sendMsg(int idxSndUser, int clientSockfd, UserID *listOfUsers, char *dirPath, int numberOfUsers, int *authSockets, message m){
	
	char msgBuf[MAX_DATA_SIZE];
	int idxRcvUsr;
	
	if(getMsgParams(msgBuf, &idxRcvUsr, m, listOfUsers, numberOfUsers)){
		return ERROR_STATUS;
	}
	
	if(authSockets[idxRcvUsr]!=-1){
		if(transmitMsg(authSockets[idxRcvUsr], idxSndUser, msgBuf, listOfUsers)){
			return ERROR_STATUS;
		}
	}
	else{
		if(writeMsgToFile(msgBuf, idxRcvUsr, idxSndUser, listOfUsers, dirPath)){
			return ERROR_STATUS;
		}
	}
...
int readMsgs(int indexOfUser, int clientSockfd, UserID *listOfUsers, char *dirPath, message m){
	...
	strcat(pathBuf, "/");
	strcat(pathBuf, listOfUsers[indexOfUser]->userName);
	strcat(pathBuf, "/Messages_received_offline.txt");
	
	fp = fopen(pathBuf, "r");
	if(!fp){
		printf("ERROR: reading offline messages file");
		return ERROR_STATUS;
	}
	...
	fp = fopen(pathBuf, "w");
	if(!fp){
		printf("ERROR: resetting offline messages file");
		return ERROR_STATUS;
	}
	fclose(fp);
	return 0;
...
int execRequest(int indexOfUser, int clientSockfd, UserID *listOfUsers, char *dirPath, int numberOfUsers, int *authSockets){
	...
	switch (m.header.opcode) {
		...
		case GETFILE:
			...
			break;
		case USERS_ONLINE:
			status = usersOnLine(clientSockfd, listOfUsers, numberOfUsers, authSockets);
			break;
		case SEND_MESSAGE:
			status = sendMsg(indexOfUser, clientSockfd, listOfUsers, dirPath, numberOfUsers, authSockets, m);
			break;
		case READ_MESSAGES:
			status = readMsgs(indexOfUser, clientSockfd, listOfUsers, dirPath, m);
			break;
		case QUIT:
			status = QUIT_STATUS;
			break;
...
int usersOnLine(int clientSockfd, UserID *listOfUsers, int numberOfUsers, int *authSockets) {
	...
	for(i=0; i<numberOfUsers; i++){
		if(authSockets[i]!=-1){
			...
			strcat(m.data, listOfUsers[i]->userName);
		}
	}
	m.header.length = strnlen(m.data, MAX_DATA_SIZE)+1;
	if (sendMessage(clientSockfd, &m)) {
